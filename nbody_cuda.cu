...
int main(int argc, char **argv) {
    ...
    cudaMalloc(&d_Fx, nBodies * sizeof(float));
    cudaMalloc(&d_Fy, nBodies * sizeof(float));
    cudaMalloc(&d_Fz, nBodies * sizeof(float));
    ...
    for (int iter = 1; iter <= nIters; iter++) {
        ...
        bodyForce<<<numBlocks, blockSize>>>(d_p, dt, nBodies, d_Fx, d_Fy, d_Fz);

        cudaMemcpy(Fx, d_Fx, nBodies * sizeof(float), cudaMemcpyDeviceToHost);
        cudaMemcpy(Fy, d_Fy, nBodies * sizeof(float), cudaMemcpyDeviceToHost);
        cudaMemcpy(Fz, d_Fz, nBodies * sizeof(float), cudaMemcpyDeviceToHost);
